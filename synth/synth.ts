...
//	                    = CharCode.Y, // should Sequence/macro exist here?
//	                    = CharCode.Z,
//	                    = CharCode.NUM_0,
//	                    = CharCode.NUM_1,
//	                    = CharCode.NUM_2,
...
export class Instrument {
    ...
    public fromJsonObject(instrumentObject: any, isNoiseChannel: boolean, isModChannel: boolean, useSlowerRhythm: boolean, useFastTwoNoteArp: boolean, legacyGlobalReverb: number = 0, jsonFormat: string = Config.jsonFormat): void {
        ...
        if (instrumentObject["volume"] != undefined) {
            if (format == "jummbox" || format == "midbox" || format == "synthbox" || format == "goldbox" || format == "paandorasbox" || format == "ultrabox" || format == "unbox") {
...
export class Song {
    ...
    private static readonly _oldestUnboxVersion: number = 0;//Not sure 0 is a valid version??? Will change to 1 if it causes errors.
    private static readonly _latestUnboxVersion: number = 2;
    // One-character variant detection at the start of URL to distinguish variants such as JummBox, Or Goldbox. "j" and "g" respectively
	//also "u" is ultrabox lol
    private static readonly _variant = 0x55; //"U" ~ Unbox
    ...
    public getVolumeCap = (isMod: boolean, modChannel?: number, modInstrument?: number, modCount?: number): number => {
        if (!isMod || modChannel == undefined || modInstrument == undefined || modCount == undefined)
            ...
            if (cap != undefined) {
                ...
                if (modulator.name == "eq filter" || modulator.name == "note filter") {//Song EQ
    ...
    public getVolumeCapForSetting = (isMod: boolean, modSetting: number, filterType?: number): number => {
        if (!isMod)
            ...
            if (cap != undefined) {
                ...
                // For filters, cap is dependent on which filter setting is targeted
                if (filterType != undefined && (Config.modulators[modSetting].name == "eq filter" || Config.modulators[modSetting].name == "note filter")) {//Song EQ
                    // type 0: number of filter morphs
                    // type 1/odd: number of filter x positions
                    // type 2/even: number of filter y positions
                    cap = Config.filterMorphCount - 1;
    ...
    public toBase64String(): string {
        ...
        buffer.push(base64IntToCharCode[Song._latestUnboxVersion]);
            ...
            if (isModChannel) {
                ...
                for (let instrumentIndex: number = 0; instrumentIndex < channel.instruments.length; instrumentIndex++) {
                    ...
                    for (let mod: number = 0; mod < Config.modCount; mod++) {
                        ...
                        if (Config.modulators[instrument.modulators[mod]].name == "eq filter" || Config.modulators[instrument.modulators[mod]].name == "note filter") {// Song EQ
                ...
                if (pattern.notes.length > 0) {
                    ...
                    for (const note of pattern.notes) {
                        ...
                        if (!isModChannel) {
                            ...
                            shapeBits.write(10, note.pins[0].size); // Modulator value. 10 bits for now = 1024 max mod value?
                        }
    ...
    public fromBase64String(compressed: string, jsonFormat: string = "auto"): void {
        ...
        // let fromSlarmoosBox: boolean;
        let fromUnbox: boolean;
        ...
        if (fromUnbox && (version == -1 || version > Song._latestUnboxVersion || version < Song._oldestUnboxVersion)) return;
        //if (fromSlarmoosBox && (version == -1 || version > Song._latestSlarmoosBoxVersion || version < Song._oldestSlarmoosBoxVersion)) return;
        //const beforeZero: boolean = version < 0;
        //const beforeOne: boolean = version < 1;
        ...
        if (fromUltraBox || fromGoldBox || fromUnbox) {
        ...
        while (charIndex < compressed.length) switch (command = compressed.charCodeAt(charIndex++)) {
			...
			if (fromJummBox && beforeThree || fromBeepBox) {
				...
				if (this.rhythm >= Config.rhythms.dictionary["÷6 (sextuplets)"].index) {
                    ...
                    //Probably should add the extra rhythm stuff here...?
				}
			}
            ...
            case SongTagCode.eqFilter: {
                if ((beforeNine && fromBeepBox) || (beforeFive && fromJummBox) || (beforeFour && fromGoldBox)) {
                    ...
                    if (fromBeepBox || typeCheck == 0) {
                        ...
                        if (fromJummBox || fromGoldBox || fromUltraBox || fromUnbox)
                        ...
                        if ((fromJummBox && !beforeFive) || (fromGoldBox && !beforeFour) || fromUltraBox || fromUnbox) {
            ...
            case SongTagCode.drumsetEnvelopes: {
                ...
                const pregoldToEnvelope: number[] = [0, 1, 2, 4, 5, 6, 8, 9, 10, 12, 13, 14, 16, 17, 18, 19, 20, 21, 23, 24, 25, 27, 28, 29, 32, 33, 34, 31, 11];
                if ((beforeNine && fromBeepBox) || (beforeFive && fromJummBox) || (beforeFour && fromGoldBox)) {
                    if((beforeTwo && fromGoldBox) || (!fromGoldBox && !fromUltraBox && !fromUnbox)){
                    ...
                    for (let i: number = 0; i < Config.drumCount; i++) {
                        let aa: number = base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
                        if((beforeTwo && fromGoldBox) || (!fromGoldBox && !fromUltraBox && !fromUnbox)) aa = pregoldToEnvelope[aa];
                        if (!fromUnbox && aa >= 2) aa++; //2 for pitch ???
            ...
            case SongTagCode.pulseWidth: {
                ...
                if (fromUltraBox && !beforeFour || fromUnbox) {
            ...
            case SongTagCode.unison: {
                if (beforeThree && fromBeepBox) {
                    ...
                    if ((fromUltraBox && !beforeFive || fromUnbox) && (instrument.unison == Config.unisons.length))  {
                ...
                if ((beforeNine && fromBeepBox) || ((fromJummBox && beforeFive) || (beforeFour && fromGoldBox))) {
                    ...
                    if (effectsIncludeNoteFilter(instrument.effects)) {
                        ...
                        if (fromBeepBox || typeCheck == 0) {
                            ...
                            if ((fromJummBox && !beforeFive) || (fromGoldBox) || (fromUltraBox) || (fromUnbox)) {
                                let usingSubFilterBitfield: number = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6) | (base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
                                for (let j: number = 0; j < Config.filterMorphCount - 1; j++) {
                                    if (usingSubFilterBitfield & (1 << j)) {
                                        // Number of control points
                    ...
                    if (effectsIncludeChord(instrument.effects)) {
                        ...
                        if (instrument.chord == Config.chords.dictionary["arpeggio"].index && (fromJummBox||fromGoldBox||fromUltraBox||fromUnbox)) {
                    ...
                    if (effectsIncludeVibrato(instrument.effects)) {
                        ...
                        // Custom vibrato
                        if (instrument.vibrato == Config.vibratos.length && (fromJummBox || fromGoldBox || fromUltraBox || fromUnbox)) {
                    ...
                    if (effectsIncludePanning(instrument.effects)) {
                        ...
                        if ((fromJummBox && !beforeTwo) || fromGoldBox || fromUltraBox || fromUnbox)
            ...
            case SongTagCode.pan: {
                if (beforeNine && fromBeepBox) {
                    ...
                    if (fromJummBox && !beforeThree || fromGoldBox || fromUltraBox || fromUnbox) {
                        instrument.panDelay = base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
                    }
                } else {
                    // Do nothing? This song tag code is deprecated for now.
            ...
            case SongTagCode.channelNames: {
                for (let channel: number = 0; channel < this.getChannelCount(); channel++) {
                    ...
                    if (beforeFour && !fromGoldBox && !fromUltraBox && !fromUnbox)
            ...
            case SongTagCode.operatorFrequencies: {
                const instrument = this.channels[instrumentChannelIterator].instruments[instrumentIndexIterator];
							...
							else if (!fromGoldBox && !fromUltraBox && !fromUnbox) {
            ...
            case SongTagCode.envelopes: {
                ...
                const unURL1toURL2Envelope: number[] = [0, 1, 2, 3, 4, 5, 6, 7, 8, 8, 9, 10, 11, 12, 13, 14]; // Do I need this???
                const instrument: Instrument = this.channels[instrumentChannelIterator].instruments[instrumentIndexIterator];
                if ((beforeNine && fromBeepBox) || (beforeFive && fromJummBox) || (beforeFour && fromGoldBox)) {
                    const legacySettings: LegacySettings = legacySettingsCache![instrumentChannelIterator][instrumentIndexIterator];
                    legacySettings.operatorEnvelopes = [];
                    ...
                    if ((fromJummBox && !beforeSix) || (fromUltraBox && !beforeFive) || (fromUnbox /* && !beforeTwo*/)) {
                    ...
                    for (let i: number = 0; i < envelopeCount; i++) {
                        ...
                        if (!fromUnbox || beforeThree) {
                            updatedEnvelopes = true;
                            perEnvelopeSpeed = Config.envelopes[aa].speed;
                            aa = Config.envelopes[aa].type; //update envelopes
                        } else if (beforeTwo && aa >= 2) aa++; //2 for random ?
                        let isTremolo2: boolean = false;
                        if ((fromUnbox && !beforeThree && beforeFour) || updatedEnvelopes) { //remove tremolo2
                            if(aa == 9) isTremolo2 = true;
                            aa = unURL1toURL2Envelope[aa]; 
                        }
                        const envelope: number = clamp(0, ((fromUnbox && !beforeThree || updatedEnvelopes) ? Config.newEnvelopes.length : Config.envelopes.length), aa);
                        let pitchEnvelopeStart: number = 0;
                        let pitchEnvelopeEnd: number = Config.maxPitch;
                        let envelopeInverse: boolean = false;
                        perEnvelopeSpeed = (fromUnbox && !beforeThree) ? Config.newEnvelopes[envelope].speed : perEnvelopeSpeed;
                        ...
                        if (!fromUnbox || beforeFour) { //update tremolo2
                            if (isTremolo2) {
                        ...
                        if (fromUnbox && beforeThree && !beforeTwo) {
                            let pitchEnvelopeCompact: number = base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
    ...
    public toJsonObject(enableIntro: boolean = true, loopCount: number = 1, enableOutro: boolean = true): Object {
        ...
        const result: any = {
            "name": this.title,
            "format": Song._format,
            "version": Song._latestUnboxVersion,
...
class EnvelopeComputer {
    ...
    public getPitchValue(instrument: Instrument, tone: Tone | null, instrumentState: InstrumentState, calculateBends: boolean = true): number {
        if (tone && tone.pitchCount >= 1) {
            ...
            //const monophonic = chord.name == "monophonic" // Maybe later
    ...
    private computeTone(song: Song, channelIndex: number, samplesPerTick: number, tone: Tone, released: boolean, shouldFadeOutFast: boolean): void {
        ...
        if (instrument.type == InstrumentType.fm || instrument.type == InstrumentType.fm6op) {
            ...
            /*if (instrument.type == InstrumentType.dutyCycle) {
                ...
                const pulseWidthStart: number = pulseWidthModStart;
