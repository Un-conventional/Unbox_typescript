...
export class SongEditor {
  ...
  /*private readonly _ringModWaveSelect: HTMLSelectElement = buildOptions(select({}), Config.operatorWaves.map(wave => wave.name));
    ...
    //private readonly _slidetickSpeedDisplay: HTMLSpanElement = span({ style: `color: ${ColorConfig.secondaryText}; font-size: smaller; text-overflow: clip;` }, "x1");
    //private readonly _slidetickSpeedSlider: Slider = new Slider(input({style: "margin: 0;", type: "range", min: "0", max: Config.modulators.dictionary["transition speed"].maxRawVol,  value: "0", step: "1" }), this._doc, (oldValue: number, newValue: number) => new ChangeSlidetickSpeed(this._doc, oldValue, newValue), false);
    //private readonly _slidetickSpeedRow: HTMLElement = div({ class: "selectRow dropFader" }, span({ class: "tip", style: "margin-left:4px;", onclick: () => this._openPrompt("slidetickSpeed") }, "‣ Spd:"), this._slidetickSpeedDisplay, this._slidetickSpeedSlider.container);
    ...
    private readonly _transitionDropdownGroup: HTMLElement = div({ class: "editor-controls", style: "display: none;" }, this._clicklessTransitionRow); //this._slidetickSpeedRow);
    ...
    private readonly _pulseWidthSlider: Slider = new Slider(input({ style: "margin: 0;", type: "range", min: "1", max: (Config.pulseWidthRange * 2), value: "1", step: "1" }), this._doc, (oldValue: number, newValue: number) => new ChangePulseWidth(this._doc, oldValue, newValue), false);
    ...
    private readonly _decimalOffsetSlider: Slider = new Slider(input({ style: "margin: 0;", type: "range", min: "0", max: "100", value: "0", step: "1" }), this._doc, (oldValue: number, newValue: number) => new ChangeDecimalOffset(this._doc, oldValue, 100 - newValue), false);
    ...
    //private readonly _dutyCycleSpeedDisplay: HTMLSpanElement = span({ style: `color: ${ColorConfig.secondaryText}; font-size: smaller; text-overflow: clip;` }, "x1");
    //private readonly _dutyCycleSpeedSlider: Slider = new Slider(input({ style: "margin: 0;", type: "range", min: 0, max: Config.modulators.dictionary["dutyCycle Speed"].maxRawVol, value:"0", step: "1"}), this._doc, (oldValue: number, newValue: number) => new ChangeDutyCycleSpeed(this._doc, oldValue, newValue), false);
    //private readonly _dutyCycleSpeedRow: HTMLElement = div({ class: "selectRow dropFader" }, span({ class: "tip", style: "margin-left:4px;", onclick: () => this._openPrompt("dutyCycleSpeed") }, "‣ Spd:"), this._dutyCycleSpeedDisplay, this._dutyCycleSpeedSlider.container);
    //private readonly _dutyCycleDropdown: HTMLElement = div({ class: "editor-controls", style: "display: none;" }, this._dutyCycleSpeedRow);
    ...
    private readonly _songTitleInputBox: InputBox = new InputBox(input({ style: "font-weight:bold; border:none; width: 98%; word-wrap: break-word; background-color:${ColorConfig.editorBackground}; color:${ColorConfig.primaryText}; text-align:center", maxlength: "63", type: "text", value: EditorConfig.versionDisplayName }), this._doc, (oldValue: string, newValue: string) => new ChangeSongTitle(this._doc, oldValue, newValue));
    ...
    */
    ...
    constructor(private _doc: SongDocument) {
        ...
        this._pwmSliderInputBox.addEventListener("input", () => { this._doc.record(new ChangePulseWidth(this._doc, this._doc.song.channels[this._doc.channel].instruments[this._doc.getCurrentInstrument()].pulseWidth, Math.min((Config.pulseWidthRange * 2), Math.max(1.0, Math.round(+this._pwmSliderInputBox.value))))) });
        ...
        if (!this._doc.song.getChannelIsMod(this._doc.channel)) {
           ...
           if  (this._wasModChannel == null || this._wasModChannel) {
            ...
            if (instrument.type == InstrumentType.pwm ||/* instrument.type == InstrumentType.dutyCycle ||*/ instrument.type == InstrumentType.supersaw) {
                ...
                this._decimalOffsetSlider.updateValue(100 - instrument.decimalOffset);
